...
function EnableResetOnConnect(props: {
    checked: any;
    handleChange: (e: SelectChangeEvent) => void;
    onChange: any;
}) {
    return (
        <Stack direction="row" alignItems="center" spacing={2} sx={{ height: 56 }}>
            <Paper elevation={0} sx={{ bgcolor: 'transparent' }}>
                <Typography sx={{ minWidth: LEVEL1_SELECT_TITLE_WIDTH }}>
                    Reset On Connect
        </Typography>
            ...
            <Stack direction="row" spacing={1} alignItems="center">
                ...
                />
                <Typography>{props.checked ? 'On' : 'Off'}</Typography>
            </Stack>
        </Stack>
    );
}
...
export default function ConnectionWidget(props: ConnectionProps) {
    ...
    const [resetOnConnect, setResetOnConnect] = React.useState<null | boolean>(
        null
    );
        ...
        async function load() {
            ...
            if (settingRegistry) {
                try {
                    ...
                    console.error(
                        `Failed to set settings for ${Attributes.plugin}\n${reason}`
                    );
                }
    ...
    useEffect(() => {
        context.current.resetOnConnect = resetOnConnect;
    }, [resetOnConnect]);
    ...
    const getJson = async () => {
        ...
        try {
            ...
            let jresetOnConnect = jsonMerge['resetOnConnect'];
            setResetOnConnect(jresetOnConnect);
        ...
        return (
            <Stack justifyContent="center" alignItems="center" sx={{ m: 2 }}>
                <StepperModeSelect
                    ...
                    updateSettings={setExtensionSettings}
                    updateControlState={handleControlState}
                />
            </Stack>
        );
    }
        ...
        return (
            ...
            >
              ...
              Protocol{' '}
                ...
                {resetOnConnect !== null && (
                    <EnableResetOnConnect
                        handleChange={handleAttnChange}
                        checked={resetOnConnect}
                        onChange={(event: any) => {
                            setResetOnConnect(!resetOnConnect);
                        }}
                    />
                )}
            </Stack>
                ...
                <MobileStepper
                    ...
                    nextButton={
                        <Button
                            ...
                            onClick={handleNext}
                            disabled={
                                activeStep === maxSteps - 1 || controlState['next'] === false
                            }
                        >
                            Next
              {theme.direction === 'rtl' ? (
